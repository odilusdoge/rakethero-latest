...
$jobseekerId = $_POST['jobseekerId'] ?? null;
...
try {
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO user_ratings (rater_id, rated_user_id, rating, comment, transaction_id) 
                               VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("iiisi", $_SESSION['user_id'], $jobseekerId, $rating, $comment, $transactionId);
        $stmt->execute();
        ...
        $stmt->bind_param("ii", $jobseekerId, $jobseekerId);
        $stmt->execute();
